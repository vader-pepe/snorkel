...
const getTwitImg = async (rawData) => {
    const browser = await puppeteer.launch({ userDataDir: userDataDir, headless: false });
...
const postToFacebook = async (user, pass, photo, caption = "Sent from API") => {
    ...
    try {
        ...
        if (loggedIn) {
            ...
            await page.waitForSelector('[data-sigil="touchable hidden-button photo-button"]');
...
const postToInstagram = async (user, pass, photo, caption = "Sent from API") => {
    const browser = await puppeteer.launch({ userDataDir: userDataDir, headless: false });
    const page = await browser.newPage();
    try {
